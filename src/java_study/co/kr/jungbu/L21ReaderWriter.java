...
public class L21ReaderWriter {
	...
	public static void main(String[] args) {
		...
		// Reader Writer가 끝에 작성되어 있으면 문자 기반 입출략 클래스 (자바)
		System.out.println("문자열을 입력하세요!");
		InputStreamReader isr = null;
		BufferedReader br = null;  
		// 버퍼링 : 8gb 메모리(Ram)를 모두 사용하면 하드디스크에 임시 저장공간을 마련해서 메모리처럼 사용 -> 컴퓨터가 멈추기 않도록 돕는 역할
		isr = new InputStreamReader(System.in);
		br = new BufferedReader(isr);  // 보조 스트림
		String inputStr = "";
		try {
			inputStr = br.readLine();
			System.out.println("입력할 문자열: " + inputStr);
		} catch (IOException e) {
			e.printStackTrace();
		}
